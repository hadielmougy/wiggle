...
public final class JdbcStorage implements Storage {
    ...
    public static final List<Migration> MIGRATIONS = List.of(
            ...
            """),
            // The event log: one append per instance lifecycle transition, written in the same
            // transaction as the transition itself, so the log never disagrees with wf_instance.
            // Delivery is pull-and-ack: a consumer is a named cursor into the seq order, and an
            // ack advances the cursor (cumulative), never touches rows. seq is DB-generated, so
            // a reader may observe seq N while an earlier, still-uncommitted transaction holds
            // N-1 -- the poll path must therefore not serve past the oldest in-flight append
            // (a short visibility grace on created_at is enough). Retention trims below
            // min(acked_seq) with an age cap, leader-only, so an abandoned cursor cannot pin
            // the log forever. payload_ver is the persisted envelope version, upcast on read
            // like the instance context.
            new Migration(15, "event-log", """
