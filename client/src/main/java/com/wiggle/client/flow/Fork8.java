...
public final class Fork8<A, B, C, D, E, F, G, H> {
    ...
    /** {@link #combine(FlowFn8<A,)} with an explicit retry policy for the combine node. */
    public <R> WiggleFlow<R> combine(FlowFn8<A, B, C, D, E, F, G, H, R> combine, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, null);
    }

    /** {@link #combine(FlowFn8<A,)} pinned to a dedicated worker queue. */
    public <R> WiggleFlow<R> combine(FlowFn8<A, B, C, D, E, F, G, H, R> combine, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), null, queue);
    }

    /** {@link #combine(FlowFn8<A,)} with both a retry policy and a dedicated queue. */
    public <R> WiggleFlow<R> combine(FlowFn8<A, B, C, D, E, F, G, H, R> combine, RetryPolicy retry, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, queue);
    }

    /** {@link #combine(FlowFn8<A,)} , queue first. */
    public <R> WiggleFlow<R> combine(FlowFn8<A, B, C, D, E, F, G, H, R> combine, String queue, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, queue);
    }

    /** {@link #combineWithContext(FlowFn9<X,)} with an explicit retry policy for the combine node. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn9<X, A, B, C, D, E, F, G, H, R> combine, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, null);
    }

    /** {@link #combineWithContext(FlowFn9<X,)} pinned to a dedicated worker queue. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn9<X, A, B, C, D, E, F, G, H, R> combine, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), null, queue);
    }

    /** {@link #combineWithContext(FlowFn9<X,)} with both a retry policy and a dedicated queue. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn9<X, A, B, C, D, E, F, G, H, R> combine, RetryPolicy retry, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, queue);
    }

    /** {@link #combineWithContext(FlowFn9<X,)} , queue first. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn9<X, A, B, C, D, E, F, G, H, R> combine, String queue, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, queue);
    }
