...
/**
 * The stage a 10-armed {@link Wiggle#allOf} returns; its combine is mandatory, because the arms ran on
 ...
 * worker startup.
 */
public final class Fork10<A, B, C, D, E, F, G, H, I, J> {

    private final Plan.Fork fork;

    Fork10(Plan.Fork fork) {
        this.fork = fork;
    }
     ...
     */
    public <R> WiggleFlow<R> combine(FlowFn10<A, B, C, D, E, F, G, H, I, J, R> combine) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), null, null);
    }

    /**
     * {@link #combine(FlowFn10)} for a merge that also needs the pre-fork context: a handler declared
     * as {@code (@Context X base, ...10 arms)}. The context parameter comes first so the type
     * arguments line up with the declaration.
     */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn11<X, A, B, C, D, E, F, G, H, I, J, R> combine) {
    ...
    /** {@link #combine(FlowFn10<A,)} with an explicit retry policy for the combine node. */
    public <R> WiggleFlow<R> combine(FlowFn10<A, B, C, D, E, F, G, H, I, J, R> combine, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, null);
    }

    /** {@link #combine(FlowFn10<A,)} pinned to a dedicated worker queue. */
    public <R> WiggleFlow<R> combine(FlowFn10<A, B, C, D, E, F, G, H, I, J, R> combine, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), null, queue);
    }

    /** {@link #combine(FlowFn10<A,)} with both a retry policy and a dedicated queue. */
    public <R> WiggleFlow<R> combine(FlowFn10<A, B, C, D, E, F, G, H, I, J, R> combine, RetryPolicy retry, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, queue);
    }

    /** {@link #combine(FlowFn10<A,)} , queue first. */
    public <R> WiggleFlow<R> combine(FlowFn10<A, B, C, D, E, F, G, H, I, J, R> combine, String queue, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, queue);
    }

    /** {@link #combineWithContext(FlowFn11<X,)} with an explicit retry policy for the combine node. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn11<X, A, B, C, D, E, F, G, H, I, J, R> combine, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, null);
    }

    /** {@link #combineWithContext(FlowFn11<X,)} pinned to a dedicated worker queue. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn11<X, A, B, C, D, E, F, G, H, I, J, R> combine, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), null, queue);
    }

    /** {@link #combineWithContext(FlowFn11<X,)} with both a retry policy and a dedicated queue. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn11<X, A, B, C, D, E, F, G, H, I, J, R> combine, RetryPolicy retry, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, queue);
    }

    /** {@link #combineWithContext(FlowFn11<X,)} , queue first. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn11<X, A, B, C, D, E, F, G, H, I, J, R> combine, String queue, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, queue);
    }
