...
public final class Fork3<A, B, C> {
    ...
    /** {@link #combine(FlowFn3<A,)} with an explicit retry policy for the combine node. */
    public <R> WiggleFlow<R> combine(FlowFn3<A, B, C, R> combine, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, null);
    }

    /** {@link #combine(FlowFn3<A,)} pinned to a dedicated worker queue. */
    public <R> WiggleFlow<R> combine(FlowFn3<A, B, C, R> combine, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), null, queue);
    }

    /** {@link #combine(FlowFn3<A,)} with both a retry policy and a dedicated queue. */
    public <R> WiggleFlow<R> combine(FlowFn3<A, B, C, R> combine, RetryPolicy retry, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, queue);
    }

    /** {@link #combine(FlowFn3<A,)} , queue first. */
    public <R> WiggleFlow<R> combine(FlowFn3<A, B, C, R> combine, String queue, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, false), retry, queue);
    }

    /** {@link #combineWithContext(FlowFn4<X,)} with an explicit retry policy for the combine node. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn4<X, A, B, C, R> combine, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, null);
    }

    /** {@link #combineWithContext(FlowFn4<X,)} pinned to a dedicated worker queue. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn4<X, A, B, C, R> combine, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), null, queue);
    }

    /** {@link #combineWithContext(FlowFn4<X,)} with both a retry policy and a dedicated queue. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn4<X, A, B, C, R> combine, RetryPolicy retry, String queue) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, queue);
    }

    /** {@link #combineWithContext(FlowFn4<X,)} , queue first. */
    public <X, R> WiggleFlow<R> combineWithContext(FlowFn4<X, A, B, C, R> combine, String queue, RetryPolicy retry) {
        return merge(StepNames.ofCombine(combine, fork.armNames, true), retry, queue);
    }
