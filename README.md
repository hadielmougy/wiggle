...
dependencies {
    ...
    // Or MySQL/MariaDB, Oracle, or SQL Server -- each is a drop-in module; the engine is
...
> (`wiggle-postgres`), MySQL/MariaDB (`wiggle-mysql`), Oracle (`wiggle-oracle`) and SQL Server
> (`wiggle-sqlserver`) all share one HikariCP-pooled, dialect-aware store (`wiggle-jdbc`); the
> engine is **detected from the JDBC URL**. With no JDBC URL it runs in-memory; with one it picks
> the provider that matches the URL. Supporting another database is a new dialect — no changes to
> the engine.
