...
- Deploy the **coordinator** (in-memory store — no external DB; not durable, fine for a lab).
...
- The coordinator runs a **single process** with an in-memory store — ephemeral, fine for a lab.
