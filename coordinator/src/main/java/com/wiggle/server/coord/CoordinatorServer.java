 ...
 * gRPC. The store is injected by the composition layer, so the backing is a deployment choice.
 *
 * <p>Leadership is the same announce-and-heartbeat election the cell engine runs
 * ({@link LeaderElection}), over the coordinator's own roster rather than the cell's -- one scheme
...
public final class CoordinatorServer implements AutoCloseable {
     ...
     * @param nodeId   this coordinator process's id: it announces itself under this in the coordinator
     *                 roster, and a multi-node coordinator elects one leader among them
     */
    public CoordinatorServer(CoordinatorStore store, int port, Tls.Options tls,
        ...
        this.election = new LeaderElection(store.election(),
                new Member(nodeId, nodeId, now, now), "coordinator",
                CoordinatorService.NODE_HEARTBEAT_INTERVAL_SECONDS * 1000L, missedHeartbeatsBeforeDead);
    ...
    public CoordinatorServer start() {
        election.start();   // settle leadership before the reconcile loop consults it
        api.start();
        reconciler.start();
    ...
    /** Closes the gRPC service, the reconcile loop, and the election. The store's lifecycle is the caller's. */
