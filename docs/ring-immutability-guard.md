...
compatibility window, no guarded no-op to carry. The CLI / `CoordinatedConnection.openEpoch` remain the sanctioned
...
| operator entry point (unchanged) | `client/**/CoordinatedConnection.openEpoch`, `wiggle open-epoch` CLI |